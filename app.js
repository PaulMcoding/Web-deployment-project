  ...
  if (req.session.username) {
    ...
    try {
      ...
      const result = await client.query(
        "SELECT * FROM car JOIN make ON car.makeid = make.makeid WHERE car.car_id = $1",
        [carId]
      );
...
app.post("/signup", async (req, res) => {
  try {
    ...
    const existingUser = await pool.query(
      "SELECT * FROM webusers WHERE user_email = $1",
      [email]
    );
    ...
    const result = await pool.query(
      "INSERT INTO webusers (user_email, user_pass) VALUES ($1, $2) RETURNING user_id",
  ...
  try {
    ...
    if (!query) {
      result = await pool.query(
        "SELECT car.*, make.makename FROM car JOIN make ON car.makeid = make.makeid"
      );
    } else {
      result = await pool.query(
  ...
  if (req.session.username) {
    ...
    if (id == 1) {
      try {
        const client = await pool.connect();
        const result = await client.query(
          "SELECT * FROM car join make using(makeid)"
        );
        const results = { results: result ? result.rows : null };
        res.send(results);
        client.release();
      ...
      try {
        const client = await pool.connect();
        const result = await client.query(
          "SELECT * FROM car join make using(makeid) where seller_id = $1",
          [id]
        );
        const results = { results: result ? result.rows : null };
        res.send(results);
...
app.post("/add", async (req, res) => {
  try {
    ...
    const makeCheck = await client.query(
      "Select * from make where makename = $1",
      [make]
    );
    ...
    if (makeCheck.rows.length > 0) {
      ...
      const makeResult = await pool.query(
        "INSERT INTO make(makename) VALUES ($1) RETURNING makeid",
        [make]
      );
    ...
    const result = await client.query(
      "INSERT INTO car (makeid, car_model, car_price, car_year, car_miles, car_location, car_desc, car_image, seller_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING car_id",
  ...
  try {
    ...
    if (ownersID !== 1) {
      ...
      const ownershipCheck = await client.query(
        "SELECT seller_id FROM car WHERE car_id = $1",
        [id]
      );

      if (
        ownershipCheck.rows.length === 0 ||
        ownershipCheck.rows[0].seller_id !== ownersID
      ) {
        res
          .status(403)
          .json({
            message:
              "Unauthorized: You do not own this car or the car does not exist.",
          });
        return;
      }
    }
    ...
    const makeCheck = await client.query(
      "SELECT * FROM make WHERE makeid = $1",
      [makeID]
    );
...
app.post("/messageSeller", async (req, res) => {
  if (req.session.username) {
    try {
      ...
      const messageResult = pool.query(
        "INSERT INTO webseller(seller_id, car_id, user_id, seller_message) VALUES ($1, $2, $3, $4)",
  ...
  try {
    ...
    if (req.session.userID) {
      ...
      const messageResult = await pool.query(
        "INSERT INTO favourite(user_id, car_id) VALUES ($1, $2)",
        [userID, carID]
      );
